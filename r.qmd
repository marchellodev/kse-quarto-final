...
sirens_map_filler <- data.frame(colors=c(), shapeISO=c())
timer <- sirens_from
...
while(timer <= sirens_to){
  ...
  for(oblast in oblasts){
    if(oblast[["shapeISO"]] %in% c("UA-65", "UA-09", "UA-43", "UA-40")){
      sirens_map_filler <- rbind(sirens_map_filler, data.frame(time=timer, colors=c("black"), shapeISO=c((oblast[["shapeISO"]]))))
    }
  }
